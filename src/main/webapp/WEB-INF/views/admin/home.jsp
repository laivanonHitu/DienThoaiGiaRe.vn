...
<h3>Tùy chọn</h3>
				<a href="<c:url value="/admin/them-san-pham"/>"
				...
				<table class="table">
					...
					<tbody>
						<c:forEach var="items" items="${products}" begin="1"
							end="${products.size()}" varStatus="loop">
							<tr>
								<td>${loop.index}</td>
								<td>${items.getName()}</td>
								<td><img src="assets/web/image/product/${items.getImage()}"
									height="50" /></td>
								<td><fmt:formatNumber type="number" maxFractionDigits="10"
										value="${items.getPrice() }" /> ₫</td>
								<td><fmt:formatNumber type="number" maxFractionDigits="10"
										value="${items.getPriceDis() }" /> ₫</td>
								<td>${items.getShortDecription()}</td>
								<td>${items.getBrand()}</td>
								<td>${items.getModifieddate()}</td>
								<td><a class="btn btn-danger"
									href="<c:url value="/xoa-san-pham-id/${items.getId()}"/>">Xóa</a></td>
								<td><a class="btn btn-info"
									href="<c:url value="/cap-nhat-san-pham-id/${items.getId()}"/>">Sửa</a></td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
